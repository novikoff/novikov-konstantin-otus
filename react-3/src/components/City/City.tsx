    ...
    return (
      <React.Fragment>
        {this.props.currentCityData && (
          <div>
            <h1>
              {this.props.currentCityData.city.name},{" "}
              {this.props.currentCityData.city.country}
            </h1>
            {this.props.favorite_arr.indexOf(Number(this.props.id)) !== -1 ? (
              <button
                className="btn  btn-danger align-self-center"
                onClick={this.eventRemoveFromFavorite}
              >
              ...
              <button
                ...
                onClick={this.eventAddToFavorite}
              >
                Add to favorite
              </button>
            )}
                  ...
                  <div className="card cityCard" key={index}>
                    <div className="card-body">
                      <h3 className="card-title text-center ">
                        {this.dateConverter(el.dt)}
                      </h3>

                      <p className="card-text">
                        Temperature: {el.main.temp.toFixed(1)}&deg;C
                      </p>
                      <p className="card-text">
                        Feels like: {el.main.feels_like.toFixed(1)}&deg;C
                      </p>
                      <p className="card-text">
                        Clouds: &nbsp;
                        {el.clouds.all < 25 ? (
                          <img
                            src="/images/sun.png"
                            title={String(el.clouds.all)}
                            alt={String(el.clouds.all)}
                            className="cloudIcon"
                          />
                        ) : el.clouds.all < 50 ? (
                          <img
...
function mapStateToProps(
  state: IState,
  ownProps: RouteComponentProps<IRouterProps>
) {
  return {
